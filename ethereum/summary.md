...
비트코인의 트랜잭션에서는 블록체인 내에서 랜덤성을 띄고 있는 값들(논스, 이전 블록의 해시, 타임스탬프)을 불러서 사용할 수 없다. 그래서 비트코인은 지급 결제 기능 외의 랜덤성을 요구하는 도박과 같은 어플리케이션을 만드는게 한계가 있다. 비트코인 스크립트로는 단순 작업 (서명, 검증) 밖에 할 수 없기 때문에, 각 트랜잭션에서 해당 블록 또는 이전 블록의 정보를 알 수 있는 방법이 없다.
=> **World States 개념을 도입해서 전지적인 관점에서 블록을 관찰할 수 있게 함**
